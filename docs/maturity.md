...
**5 of 7 predicates met.** A predicate is met when every story named for it is `done` — the stories are the definition, so this table cannot drift from the board.
...
110 stories done. `blocked` counts as open: a story parked on a dependency is distance, not progress.
