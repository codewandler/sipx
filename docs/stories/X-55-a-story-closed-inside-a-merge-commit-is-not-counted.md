...
## Acceptance
- [ ] **Reproduce it.** `history_story_fact_days` walks
...
- [ ] **The same hole exists for `filed`.** `--diff-filter=A --name-only` is a separate `git log`
...
- [ ] **Decide which way to close it, and say why.** Either count merge diffs — `--diff-merges=1`
...
- [ ] **The diagnostic, if that is the route, names the cause.** `X-49`'s lesson was that
...
- [ ] **The report must not need hand repair.** Recovering from this took editing the generated
