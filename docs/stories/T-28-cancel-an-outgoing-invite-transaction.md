...
predicate:
note: requested by sipx-clstr PX-12 — the UA has a private builder, while a proxy cannot cancel one branch through Handle
---
...
- [ ] Failing-first test: `a_forwarding_element_can_cancel_one_outgoing_invite_branch` runs a real
      ...
      not compile against `v0.10.0`, whose public handle has only `send` and `send_directly`.
...
- At `v0.10.0`, `sipx-transport/src/endpoint.rs` stores the outgoing transaction's `Target` and
  exposes its event stream as `Responses`, but exposes no command that acts on that transaction.
  `sipx-call/src/call.rs::send_cancel` is private and rebuilds the request for the UA-shaped call
  path. `sipx_sip::TransactionKey::for_cancelled_invite` already supplies the generic association
  rule for the receiving half.
