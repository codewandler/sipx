...
The current published prerelease is **`1.0.0-beta.6`, not stable 1.0**. `A-35` and the integrated
post-beta.5 tree are published; exact registry, CLI, Pages and GitHub release evidence agrees, so all
six announcement predicates are met. The retained load evidence covers the current endpoint
direction only, and planned browser/media/application work is not a runtime claim. The
