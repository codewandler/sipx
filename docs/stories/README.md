...
and a scripted app. `A-1`, `A-2`, `A-7` and the release-surface hardening in `A-9` are done; app-host
phase 1 is closed, and `A-3`/`A-4` are the next host phase. Then the
...
- **M9 — Bridgeable.** Two thirds done: `S-19` UPDATE and `C-2` early media are closed; `C-1` two
  ...
  them.
- **M11 — Attestable is delivered.** The STIR service (`S-20`), diversion history (`S-21`), overload
  control (`T-22`) and live-call identity proof (`S-34`) are all closed.
