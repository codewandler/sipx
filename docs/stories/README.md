...
The current published prerelease is **`1.0.0-beta.4`, not stable 1.0**. `A-24` is cutting the
complete post-beta.4 tree as the new immutable `1.0.0-beta.5` candidate; announcement predicate 6
stays open until exact registry, CLI, Pages and GitHub release evidence agrees. The alpha-integrity
table remains **7 of 7**.
...
## Now (in progress)
- [A-24 — Publish and verify 1.0.0-beta.5](A-24-publish-and-verify-beta5.md) · Application · next immutable beta from current main · exact registry, CLI, Pages and prerelease evidence
