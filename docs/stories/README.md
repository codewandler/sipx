...
The selected post-beta.4 implementation wave is **M13 — Endpoint-complete**: eleven stories across
six epics, queryable as `parity-wave-1`. X-98 is in progress as the late-M13 load-contract
dependency; P-15 consumes it after X-75, while the actual comparative run remains M14. M15 separately
tracks the audio-only browser SDK, WASM kernel, browser-native WebRTC adapter and public demo.
