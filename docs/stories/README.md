...
The current published prerelease is **`1.0.0-beta.5`, not stable 1.0**. `A-24` and the complete
post-beta.4 wave are done; exact registry, CLI, Pages and GitHub release evidence agrees, so all six
announcement predicates are met. The alpha-integrity table remains **7 of 7**. This publication is
not broader publicity, and the credentialed live realtime-endpoint proof remains pending.
