...
The current release target is **`1.0.0-beta.4`, not stable 1.0**. The selected wave is exactly ten
stories carrying the query-only `beta4` area tag. It is measured at **5 of 6** all-or-nothing
announcement predicates, with only immutable publication evidence open. The alpha-integrity table
is **7 of 7** after closing the SRTCP replay gap and the wave's input-hardening work.

`A-15` is the release capstone now in progress; the other nine stories and the `M-38` epic exit are
done. The wave delivers a fail-closed browser-audio profile, one nominated media component, an
independent Opus proof and the highest-demand non-ICE connectivity path, with malformed ingress,
entropy and SRTCP replay invariants pinned. Stable `1.0.0`, TURN, video and the second
PCM/G.722/jitter lane remain separate decisions rather than silent beta.4 scope growth.
