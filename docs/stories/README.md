...
The current release boundary is **`1.0.0-rc.1`, not stable 1.0**. `A-37` owns its protected
publication and exact registry, CLI, Pages, portable-asset and GitHub release evidence. All six
historical beta-announcement predicates are met. Compatible endpoint-responder runs now cover both
selected subjects under one bounded profile; their intervals overlap at the tested ceiling, so no
ranking is claimed. The alpha-integrity table remains **7 of 7**, while stable promotion still lacks
...
The delivered post-beta.4 wave is **M13 — Endpoint-complete**: fifteen stories across nine epics,
queryable as `parity-wave-1`, with all 36 integration-gate steps green. X-97 discovered three
additional endpoint gaps and pulled the bounded responder plus its prerequisite profile forward;
M14 retains the actual comparative run.
...
## Now (in progress)
- [A-37 — Publish and verify 1.0.0-rc.1](A-37-publish-and-verify-rc1.md) · Application · first release candidate · reviewed docs, exact registry packages and portable artifacts
...
- [S-24 — Learn who is registered, with the registration event package](S-24-learn-who-is-registered.md) · Signalling · live bounded UAC and registration-event discovery are integration-gate proven
...
- [S-35 — Accept an inbound subscription from a socket](S-35-accept-an-inbound-subscription-from-a-socket.md) · Signalling · RFC 6665 notifier is reachable from a live endpoint and shares the bounded subscription store
