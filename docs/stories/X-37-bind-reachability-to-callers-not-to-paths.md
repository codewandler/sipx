...
- [ ] The check answers "can something above the implementing crate select this capability" by
      ...
      and 8122**.
- [ ] **The dead-branch limit is closed.** A path check is satisfied by citing a call-layer file that
...
- [ ] **The `transport` layer becomes adjudicable.** `X-33` measured it and declined it, correctly: it
...
- [ ] **RFC 5626 and 8599's `uac` claims are adjudicated**, either way. `X-33` flagged them as
...
- [ ] **The `layer` relabel escape closes properly.** `X-33` pinned it for rows citing `sipx-media`,
...
- [ ] **Still no suppression list, under any name.** Held twice, by `X-30` and by `X-33`. A row that
...
- [ ] Failing-first test: a fixture row citing a call-layer file whose relevant branch is dead passes
      `--check` today. RFC 8122's pre-demotion state is the worked example. Name the test.

## Progress
- Not started. Filed at `X-33`'s integration, from its explicit request: *"Left deliberately unbuilt:
