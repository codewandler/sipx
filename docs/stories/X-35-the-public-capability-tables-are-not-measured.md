...
- [ ] **Opus is advertised as a stack capability and no call can select it.** `README.md:26`,
...
- [ ] **"Bridges and conferences" is sold as a phone capability in five places; two `Call`s cannot be
...
- [ ] **DTLS-SRTP is described as reachable "by building your own capabilities", and the workaround
...
- [ ] **`X-26`'s removed untruth survives in the fourth front door its guard does not read.**
...
- [ ] `README.md:30` — "No Outbound, Path, GRUU or push yet". All four exist with public entry points
...
### The missing warning
- [ ] **The CLI cannot reach a secure transport and nothing says so.** `sipx-cli/src/dial.rs:32` and
...
- [ ] **Generalise `check-audio-claims.py` from codecs to front doors.** Every finding above is one
      ...
      first three.
- [ ] The check runs in `./scripts/gate.py` and in CI, per `X-22`'s parity rule.
- [ ] **No suppression list**, under any name — `X-30` held that line and it is why its check is
      worth having.
- [ ] Failing-first test: `README.md:114`'s "RFC 4733 DTMF" passes
      `check-audio-claims.py --check` today at exit 0. Name the test that makes it fail.

## Progress
- Not started. Filed from two independent read-only sweeps that never met: one over `README.md` +
