...
**Reason (RFC 3326)**
- [ ] `Reason` is populated rather than only preserved: on a CANCEL and on a BYE, with `protocol` of
...
- [ ] The §3.1 case is expressible: cancelling the remaining branches of a forked request with the
...
- [ ] `Reason` is placed only where §2 permits it: "in any request within a dialog, in any CANCEL
...
- [ ] `History-Info` is a typed list header (§5): `hi-entry` values of an `hi-targeted-to-uri` with
...
- [ ] Indexing follows §10.3: the first index "MUST be set to 1"; forwarding appends "the dot
...
- [ ] `Supported: histinfo` (§6.1) is offered when the caller wants the history back in the response,
      and the history is returned in responses other than 100 (§9.3).
- [ ] The `Reason` for a hop is carried where §10.2 puts it — "in the 'headers' component of the
...
- [ ] Privacy is implemented, not skipped: a `Privacy` header field with `history` or `header` means
...
- [ ] The registry entries for RFC 7044 move off "not started" and RFC 3326 off `syntax only`, with
...
- [ ] Failing-first test: `a_retargeted_request_carries_the_previous_target_and_the_reason_it_moved`.

## Progress
- Not started. RFC 7044 is "not started" and RFC 3326 is `syntax only` — parsed and preserved, and
  "sipx does not populate it on CANCEL or BYE".
