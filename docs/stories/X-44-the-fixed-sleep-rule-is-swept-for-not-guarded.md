...
## Acceptance
- [x] **The claim currently has nothing behind it.** `CHANGELOG.md`'s `0.12.0` entry for `X-29` says
...
- [x] **Two instances appeared inside one wave, which is the argument for the story.**
      ...
      checks elsewhere.
- [x] **The rule is already written; make it executable.** `docs/designs/media.md:213-216`: "a fixed
...
- [x] **The three legitimate categories keep passing, and are named at their sites.** `X-29` identified
...
- [x] **The guard cannot be satisfied by a rename.** State how it identifies the shape rather than the
...
- [x] **`gate.py --check` accounts for the new step**, and the step is a CI job or declared CI-only
      with a reason, per the property `X-22` established.
- [x] Failing-first test: extend `scripts/test-gate.py` with a case that plants a fixed-sleep assertion
