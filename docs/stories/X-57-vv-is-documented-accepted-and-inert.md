...
## Acceptance
- [ ] **`-vv` reaches DEBUG.** `crates/sipx-cli/src/main.rs:74` counts verbosity as
...
- [ ] **The failing-first test is the flag as documented.** Run a real call with `-vv` and assert
...
- [ ] **Decide what `-vvv` and beyond mean**, and whether repeated short flags should be counted by
      character rather than by argument. `-v -v -v` and `-vvv` must not disagree.
- [ ] **The inertness is worth its own assertion.** Even parsed correctly, `-v` alone produces
