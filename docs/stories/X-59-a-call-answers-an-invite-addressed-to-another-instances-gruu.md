...
## Acceptance
- [x] **Reproduce it.** An INVITE whose Request-URI is instance one's public GRUU, delivered to
...
- [x] **Decide, and say why in the spec rather than only here.** Two defensible answers and the
      story is choosing one:
      - **Refuse.** A UA that recognises its own GRUU can refuse one that is not — a `404` is the
        shape RFC 5627 §6.1 gives an unresolved GRUU — which makes individual callability hold end
        to end even when something upstream misroutes.
      ...
      the absence of one.
- [x] **The demonstration follows the decision.** If refusal is chosen, `X-52`'s
...
- [x] **`docs/roadmap.md`'s M10 record matches whichever it is.** The Delivered entry and the "Where
...
- Reproduced failing-first in
  `each_of_two_registrations_of_an_address_of_record_is_called_individually`: the second instance
  answered an INVITE bearing the first instance's public GRUU and G.711 audio crossed both ways.
- Chose refusal and recorded the boundary and decision table in `docs/specs/gruu.md`. A
  `UserAgent` which has learned at least one GRUU now consumes an initial INVITE bearing `gr` when
  it matches neither learned URI and sends `404 Not Found`; an own-GRUU or AOR INVITE remains for
  the call layer. This mirrors RFC 5627 §6.1's unresolved-GRUU response without claiming sipx is
  the registrar or authoritative proxy.
- The strengthened `X-52` test now bypasses its routing double deliberately, sees the 404 on the
  wrong flow, and then proves both correctly routed calls still answer and carry audio.
