...
**The signal (RFC 7339)**
- [ ] The four `Via` header field parameters of §4 are parsed and generated with the right party
...
- [ ] `oc-validity` defaults to 500 milliseconds when absent (§4.3), and a value of **0** means the
...
- [ ] `oc-seq` is used to discard a stale report: responses arrive out of order, and an older `oc`
      applied after a newer one silently undoes the newer one.
- [ ] As a client, sipx honours what it is told: "The SIP client MUST NOT forward more requests to a
      SIP server than allowed by the current 'oc' and 'oc-algo' parameter values" (§5.5).
- [ ] The loss-based algorithm (§7.2) is implemented as the default, with the two message categories
...
- [ ] Prioritisation is possible, not hardcoded: §5.10.1's "A SIP client SHOULD honor the local policy
...
- [ ] `rate` is offered in the `oc-algo` list (§3.3) and the `oc` value under it is a request rate per
      second rather than a loss percentage (§3.4).
- [ ] Pacing conforms to "the upper bound of 1/T messages per second" (§3.5.1), with the burst
      tolerance parameter exposed rather than fixed.

**Where it lands**
- [ ] The shed path is the one `T-19` builds: a request shed under overload is counted, and now also
      reported upstream through `oc` rather than only answered 503. `T-19`'s test must still pass.
- [ ] The registry entries for RFC 7339 and RFC 7415 move off "not started" in the same change, and
      RFC 7339's note stops describing sipx's 503 as the current behaviour.
- [ ] Failing-first test: `a_client_told_to_reduce_by_half_forwards_half_as_many_requests`.

## Progress
- Not started. `compliance.md`: "sipx answers 503 when its application queue is full, which is the
  behaviour 7339 exists to improve on." RFC 7415 is "the rate-based half […] Depends on 7339."
