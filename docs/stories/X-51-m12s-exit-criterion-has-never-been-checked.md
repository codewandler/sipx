...
## Acceptance
- [x] **Each of the four `Done when` clauses is checked against a named test, script or CI job**, not
...
- [x] **The trap `X-50` fell into is avoided explicitly.** All four stories being `done` is what
      ...
      and look.
- [x] **The third clause is the one to read hardest.** `X-18` counted transport discards and
...
- [x] **Whatever the answer, `docs/roadmap.md` records it** — M12 moved to Delivered with its
...
- [x] If M12 is reached, the ordering note under it — "last, and for a reason that is not
