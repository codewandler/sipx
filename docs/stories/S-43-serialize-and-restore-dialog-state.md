...
- [x] A spec cites RFC 3261 §§12.1–12.2 and defines a versioned snapshot containing dialog IDs,
...
- [x] Snapshot encoding is deterministic, bounded and schema-versioned. Unknown versions, missing
...
- [x] Runtime-only state is excluded: endpoint handles, sockets, transactions, timers, spawned work,
      credentials, private keys, media keys and entropy are never serialized.
- [x] Restoration requires the host to inject a fresh endpoint/media policy and explicit current
      time inputs; expired session state is rejected or surfaced as an immediate fired-timer action,
      never revived silently.
- [x] Byte-level round-trip and hostile-snapshot vectors prove CSeq monotonicity, route order,
...
- [x] The public docs state that snapshot durability/distribution belongs to the host, and
      `./scripts/gate.py` is green.

## Progress

- 2026-08-05: the normative snapshot and restore contract was written before the codec or runtime
  attachment API.
- 2026-08-05: the `SXD1` codec, atomic fresh-driver attachment, security/timer guards, hostile-byte
  matrix and a live post-restore re-INVITE proof are implemented. Focused verification and all 36
  corrected integration-gate steps are green.
