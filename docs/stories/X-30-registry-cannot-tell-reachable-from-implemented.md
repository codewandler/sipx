...
## Acceptance
- [ ] A mechanical check that an entry claiming a role cites evidence at or above the layer that
...
- [ ] The check runs in `./scripts/rfc-report.py --check`, which is already a gate step and a CI
      job, so a new over-claim fails the gate rather than being found by the next story to touch
      that code.
- [ ] Every row the new check rejects is corrected in the same commit — or the check is narrowed,
      with the reason recorded. Landing a check plus a suppression list is how this stops working.
- [ ] Failing-first test: a fixture registry entry claiming both roles while citing only a leaf
