...
note: M9 · RFC 3960 · S-12 built the early offer/answer and stops short of using it
---
...
## Acceptance
- [ ] A UAS can put a session description in a reliable provisional response and start its media
...
- [ ] A UAC that answers such an offer in its PRACK (already built by `S-12`) starts receiving and
      rendering media on the early dialog rather than discarding it until the 2xx.
- [ ] The application is told which it is hearing. RFC 3960 §3.2: a UAC "is supposed to generate
...
- [ ] The early media session becomes the confirmed one on the 2xx without re-keying, re-binding or
      a gap in the stream. A second session created at answer time is a clip the user hears.
- [ ] An early dialog that dies — 4xx/5xx/6xx, CANCEL, or a fork losing the race — tears its media
      session down, with no socket and no task left behind. `M-11`'s `Drop` discipline is the
      precedent.
- [ ] Only the gateway model (RFC 3960 §3) is in scope, and the story says so: one offer/answer on
      the INVITE's own early dialog.
- [ ] Failing-first test: `a_caller_receives_early_media_before_the_call_is_answered`.

## Progress
- Not started. `S-12` built everything up to the offer: a reliable provisional establishes a dialog,
  an offer in it is answered in the PRACK, and then nothing starts a media session.
