...
| [4568](https://www.rfc-editor.org/rfc/rfc4568) | SDP Security Descriptions for Media Streams (SDES) | 🟡 partial | uac, uas | [srtp](specs/srtp.md) | `a=crypto` offered and answered, and **only over a secure signalling path** — §7.1 makes that a condition of use, and `Crypto::offer` returns nothing without it rather than leaving the check to every caller. No MKI, no key lifetimes, no session parameters. §5.1.2's tag is not echoed in the answer and §5.1.3's echo is not verified, both MUSTs; `M-25` found them and `docs/specs/srtp.md` §12.3 states what they should be. |
