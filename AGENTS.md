...
private helper are the same thing — and it reads `src/` as well as `tests/`, because `X-40`'s
instance of the defect was written in production code and most of this workspace's tests live in
`#[cfg(test)]` modules beside it. **A wait it reports is either converted into a wait for the thing,
or classified at the call site**: which of the four questions the duration answers, in the script's
own words. There is no suppression list, under any name — the rule had been swept for twice and
enforced by nothing, and two violations landed in the wave after the second sweep (`X-44`).
