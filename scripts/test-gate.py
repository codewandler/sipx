#!/usr/bin/env python3
"""Tests for the gate entry point and its drift check (story `X-22`).
...
So the thing under test here is not "does the gate run". It is the two ways that defect recurs:
a CI job the gate never learns about, and a `AGENTS.md` gate block that grows its own copy of the
list and falls behind. Both are assertions about real repository files rather than about fixtures,
because a check that only ever sees its own fixtures is the same class of mistake one level up.
"""
